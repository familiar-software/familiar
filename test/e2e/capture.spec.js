...
const { CAPTURES_DIR_NAME, CAPTURE_FILENAME_PREFIX, JIMINY_BEHIND_THE_SCENES_DIR_NAME } = require('../../src/const');
...
test('capture flow saves a screenshot under the context folder', async () => {
    ...
    try {
        ...
        const captureFiles = fs.readdirSync(capturesDir).filter((filename) => filename.endsWith('.png'));
        ...
        const pattern = new RegExp(`^${prefix}\\d{4}-\\d{2}-\\d{2}_\\d{2}-\\d{2}-\\d{2}-\\d{3}\\.png$`);
