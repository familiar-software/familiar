...
-   The Settings window includes a Context Graph sync action that writes summaries to `<contextFolderPath>/jiminy-behind-the-scenes/context-tree.json` and a prune action that deletes it.
-   Auto-launch on login is enabled via Electron login item settings.
-   The tray menu includes a **Capture Selection** overlay (drag to select, release to capture) that saves a PNG to `<contextFolderPath>/jiminy-behind-the-scenes/jiminy-captures`. It requires macOS Screen Recording permission in **System Settings > Privacy & Security > Screen Recording**.
...
-   **Capture exclusions:** The `jiminy-behind-the-scenes` folder (including `jiminy-captures`) is ignored during scans.
...
-   **Output:** Writes a single JSON graph to `<contextFolderPath>/jiminy-behind-the-scenes/context-tree.json` with nodes keyed by stable ids.
...
- context graph should ignore jiminy-behind-the-scenes folder
