...
if [[ "$ARCH_INFO" != *"arm64"* || "$ARCH_INFO" != *"x86_64"* ]]; then
  echo "error: apple-vision-ocr must be universal (arm64 + x86_64). got: ${ARCH_INFO:-unknown}" >&2
  exit 1
fi
...
for arch in arm64 x86_64; do
  ...
  if [[ "$MINOS" != 14.* ]]; then
    echo "error: apple-vision-ocr minos must be 14.x for $arch, got: ${MINOS:-unknown}" >&2
    exit 1
  fi

  if otool -L -arch "$arch" "$OUT" | grep -q "libswift"; then
    echo "error: apple-vision-ocr unexpectedly links Swift runtime dylibs for $arch" >&2
