...
const computeNextCheckDelay = ({ now, lastCheckedAt, delayMs }) => {
  ...
  return Math.max(UPDATE_CHECK_INTERVAL_MS - elapsed, baseDelay);
};
...
const scheduleRecurringUpdateCheck = ({ delayMs = STARTUP_CHECK_DELAY_MS } = {}) => {
  if (!enabled) {
    return { scheduled: false };
  }

  if (scheduledTimeoutId || scheduledIntervalId) {
    console.log('Recurring update checks already scheduled');
  ...
  console.log('Scheduling recurring update check', {
    delayMs: nextDelay,
    intervalMs: UPDATE_CHECK_INTERVAL_MS,
  });
  scheduledTimeoutId = setTimeout(() => {
    scheduledTimeoutId = null;
    void checkForUpdates({ reason: 'scheduled' });
